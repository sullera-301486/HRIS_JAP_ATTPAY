    ...
    namespace HRIS_JAP_ATTPAY
    {
        ...
        private async void LoadAttendanceSummary(DateTime date)
        {
            try
            {
                ...
                int lateCount = 0;
                int absentCount = 0;
                int dayOffCount = 0; // Track day off employees
                        ...
                        if (attendanceRecord?.Object != null)
                        {
                            ...
                            if (attendanceDict != null && attendanceDict.Any())
                            {
                                ...
                                if (matchesDate && !string.IsNullOrEmpty(employeeId) && allEmployees.Contains(employeeId))
                                {
                                    ...
                                    // Check for Day Off status first
                                    if (lowerStatus.Contains("day off") || lowerStatus == "dayoff")
                                    {
                                        dayOffCount++;
                                        continue; // Skip counting day off employees in regular categories
                                    }
                ...
                // Update UI with counts (excluding day off employees from total)
                int employeesToCount = totalEmployees - dayOffCount;
                labelTotalNumber.Text = employeesToCount.ToString();
                    ...
                    foreach (var attendanceRecord in attendanceRecords)
                    {
                        if (attendanceRecord?.Object != null)
                        {
                            ...
                            if (attendanceDict != null && attendanceDict.Any())
                            {
                                ...
                                // EXCLUDE DAY OFF EMPLOYEES
                                bool isDayOff = status.ToLower().Contains("day off") || status.ToLower() == "dayoff";

                                if (matchesDate && !string.IsNullOrEmpty(employeeId) && status.ToLower() == "absent" && !isDayOff)
                                {
                                    // Get employee name
                                    string fullName = "N/A";
        ...
        private void setTodoDataGridViewAttributes()
        {
            ...
            dataGridViewTodo.AllowUserToAddRows = false;
            dataGridViewTodo.AllowUserToDeleteRows = false;
            dataGridViewTodo.AllowUserToOrderColumns = false;
            dataGridViewTodo.AllowUserToResizeRows = false;
            ...
            dataGridViewTodo.Columns["ColumnTask"].ReadOnly = true; // Make column read-only

            // Due Date column
            dataGridViewTodo.Columns.Add("ColumnDueDate", "Due Date");
            dataGridViewTodo.Columns["ColumnDueDate"].Width = 150;
            dataGridViewTodo.Columns["ColumnDueDate"].ReadOnly = true; // Make column read-only
            ...
            deleteColumn.ReadOnly = true; // Make column read-only
