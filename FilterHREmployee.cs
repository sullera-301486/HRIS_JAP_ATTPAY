...
namespace HRIS_JAP_ATTPAY
{
    ...
    {
        // Events so HREmployee can subscribe
        public event Action<HRFilterCriteria> FiltersApplied;
        ...
        private async void LoadDepartmentsAndPositions()
        {
            try
            {
                ...
                System.Diagnostics.Debug.WriteLine($"HR Filter: Loaded {departments.Count} departments and {positions.Count} positions");
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"HR Filter: Failed to load departments/positions: {ex.Message}");
        ...
        private async Task<Dictionary<string, (string Department, string Position)>> TryGetEmploymentInfoByIndex()
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"HR Filter: Index approach failed: {ex.Message}");
            }
        ...
        private async Task<Dictionary<string, (string Department, string Position)>> TryManualEmploymentInfoParsing()
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"HR Filter: Manual parsing failed: {ex.Message}");
            }
        ...
        {
            var filters = new HRFilterCriteria
            {
